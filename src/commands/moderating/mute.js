...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        let muterole = message.guild.roles.find(r => r.name === "sclavii nu au voie sa vb") //look to see if muted role already exists

        if(!muterole) { //if not
            try{
                muterole = await message.guild.createRole({
                    name: "ia mancat pisica limba",
                    color: "#514f48",
        ...
        if(mutee.roles.has(muterole.id)) return message.channel.send(`${mutee} e deja mut!`); //check if user is already muted
        mutee.addRole(muterole.id); //if not add role
        return message.channel.send(`${mutee} acuma pastreaza tacerea bai crackere! Motiv: ${reason}`);
    }
}
