...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if(rMember.roles.has(role.id)) {
            return message.channel.send(`**${rMember.displayName} deja are rolu!**`)
        } else {
            try {
                await rMember.addRole(role.id);
                message.channel.send(`**rolu, ${role.name}, a fost adaugat ${rMember.displayName}.**`); //if successful this message
            } catch(e) {
